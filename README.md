...
| 이름 | 화면 이름 | 값 |
|---|---|---|
| `scope` | 동반 범위 | `ALL_AREA` 전 구역 · `PARTIAL` 일부 구역 · `UNKNOWN` 알 수 없음 |
...
| `scope` | `UNKNOWN` — 범위를 말했으나 전 구역인지 일부인지 가릴 수 없었음 | `null` |
...
외부 API 키가 없어 이 파일이 짧습니다. 비밀번호는 계정 10개가 같은 값을 쓰므로 여기 두지 않고 1계층이 환경변수로 받습니다.
